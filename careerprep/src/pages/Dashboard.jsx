...
export default function Dashboard() {
  ...
  const [careerTipLinks, setCareerTipLinks] = useState({});
   ...
   const loadTipLinks = async () => {
    try {
      const { data, error } = await supabase
        .from('career_tip_links')
        .select('*');
      ...
      if (data) {
        const links = {};
        data.forEach(item => {
          links[item.career_id] = item.tip_link;
        });
        setCareerTipLinks(links);
      }
    } catch (error) {
      console.error('Error loading tip links:', error);
    }
  };

   const saveTipLink = async (careerId, value) => {
    try {
      ...
      console.error('Error saving tip link:', error);
    }
  };
  ...
  const handleTipLinkChange = async (careerId, value) => {
  ...
  await saveTipLink(careerId, value);
};
  ...
  useEffect(() => {
  checkUser();
  loadTipLinks(); // Add this line
}, []);


  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 via-green-50 to-gray-50 relative overflow-hidden">
      {/* Decorative Background Elements - Enhanced */}
      <div className="absolute top-0 left-0 w-[600px] h-[600px] bg-green-200 rounded-full opacity-30 -translate-x-1/3 -translate-y-1/3 blur-3xl"></div>
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-green-300 rounded-full opacity-20 translate-x-1/3 -translate-y-1/4 blur-2xl"></div>
      <div className="absolute bottom-0 left-1/4 w-[400px] h-[400px] bg-green-200 rounded-full opacity-25 -translate-y-1/2 blur-3xl"></div>
      <div className="absolute bottom-0 right-0 w-[550px] h-[550px] bg-green-100 rounded-full opacity-30 translate-x-1/4 translate-y-1/3 blur-3xl"></div>
      ...
      <div className="container mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Sample Resume Preview - Floating Design */}
        <div className="mb-8">
          ...
          {/* Resume Paper Container - Now Floating */}
